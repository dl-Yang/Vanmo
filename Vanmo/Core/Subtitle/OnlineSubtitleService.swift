...
protocol OnlineSubtitleProvider: Sendable {
    var name: String { get }
...
actor OnlineSubtitleService {
    ...
    func download(_ result: OnlineSubtitleResult, for item: MediaItem) async throws -> URL {
    ...
    func cachedSubtitleURLs(for item: MediaItem) throws -> [URL] {
        let directory = try cacheDirectory()
...
enum OnlineSubtitleError: LocalizedError {
...
struct ConfigurableOnlineSubtitleProvider: OnlineSubtitleProvider {
    let name: String
    let endpoint: URL?
...
struct OpenSubtitlesProvider: OnlineSubtitleProvider {
    ...
    static func testLogin() async throws -> OpenSubtitlesLoginResponse {
...
enum OpenSubtitlesCredentialStore {
    static let enabledKey = "subtitle.opensubtitles.enabled"
    static let preferredLanguageKey = "subtitle.preferredLanguage"
    ...
    static func saveCredentials(apiKey: String, username: String, password: String) throws {
    ...
    static func saveSession(token: String, baseURLHost: String) {
...
struct OpenSubtitlesLoginResponse: Decodable, Sendable {
    let token: String
    let baseURL: String
    let user: OpenSubtitlesUser
...
struct OpenSubtitlesUser: Decodable, Sendable {
...
struct ShooterSubtitleProvider: OnlineSubtitleProvider {
    private let provider: ConfigurableOnlineSubtitleProvider

    init(endpoint: URL? = UserDefaults.standard.url(forKey: "subtitle.provider.shooter.searchEndpoint")) {
...
struct SubhdSubtitleProvider: OnlineSubtitleProvider {
    private let provider: ConfigurableOnlineSubtitleProvider

    init(endpoint: URL? = UserDefaults.standard.url(forKey: "subtitle.provider.subhd.searchEndpoint")) {
