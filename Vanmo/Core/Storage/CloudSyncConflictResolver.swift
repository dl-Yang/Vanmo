...
enum CloudSyncConflictResolver {
    ...
    static func mergeProgress(into item: MediaItem, position: TimeInterval, playedAt: Date) {
    ...
    static func mergeFavorite(into item: MediaItem, isFavorite: Bool, updatedAt: Date) {
        guard item.isFavoriteCloudSynced else { return }
        _ = updatedAt
        item.isFavorite = isFavorite
    }

    static func mergeWatched(into item: MediaItem, isWatched: Bool, updatedAt: Date) {
        guard item.isProgressCloudSynced else { return }
        _ = updatedAt
        if isWatched {
            item.isWatched = true
    ...
    static func mergeConnection(_ local: SavedConnection, with remote: SavedConnection) {
    ...
    static func mergeBookmark(_ local: FolderBookmark, with remote: FolderBookmark) {
    ...
    @MainActor
    static func mergePendingConflicts(in context: ModelContext) throws {
...
enum CloudSyncMediaEligibility {
    @MainActor
    static func isMediaServerItem(_ item: MediaItem, in context: ModelContext?) -> Bool {
        guard let context,
    ...
    @MainActor
    static func isMediaServerConnection(_ connectionId: UUID, in context: ModelContext?) -> Bool {
