...
public actor SubtitleManager {
    ...
    public func load(from data: Data, format: SubtitleFormat) throws {
        let parser: SubtitleParser
        switch format {
        case .srt: parser = SRTParser()
        case .vtt: parser = VTTParser()
    ...
    public func setDelay(_ newDelay: TimeInterval) {
        delay = newDelay
    }
    ...
    public static func findSubtitleFiles(for videoURL: URL) -> [URL] {
