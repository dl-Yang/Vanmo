...
/// HTTP Range 解析结果。
public enum HTTPRangeSpec: Equatable {
...
public enum HTTPProtocolHandler {

    public struct ParsedHTTPRequest {
        let method: String
        let path: String
        let headers: [String: String]
    }

    public static func parseRequest(_ headerData: Data) -> ParsedHTTPRequest? {
    ...
    /// 解析 `Range:` 头（仅 `bytes=`）。
    public static func parseRangeHeader(_ value: String) -> HTTPRangeSpec? {
    ...
    public static func streamToken(from path: String) -> String? {
    ...
    public static func build404() -> Data {
        let header =
    ...
    public static func build400() -> Data {
        let header =
