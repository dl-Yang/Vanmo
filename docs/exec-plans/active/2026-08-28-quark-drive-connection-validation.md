# Quark Drive Connection Validation
...
Record that the Quark Drive entry remains an official-cloud placeholder on both Vanmo iOS and VanmoMac. A pass means save stays disabled and the type is not used as a working source.
...
- Implementing Quark official APIs
- Unofficial APIs
...
1. Launch iOS Vanmo with `./run_device.sh --simulator` or a signed device. Open Files and choose Quark Drive.
...
- Editing a leftover historical row could still hit `unsupportedProtocol`; that is not a pass for a working Quark client.
