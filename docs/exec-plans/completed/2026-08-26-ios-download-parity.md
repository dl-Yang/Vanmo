...
- **2026-08-26:** Fast `./init.sh` passed all four stages then in use (`VanmoCore` tests, CloudKit/multiplatform static check, harness documentation check, and the later-retired UI CLI static check). `xcodegen generate` included `DownloadManagementView.swift`. Focused `./scripts/check-app-build.sh ios-simulator` passed (`BUILD SUCCEEDED`; evidence `build/app-build-evidence/runs/20260826-173918-36436`). A single-step tap on `下载管理` could not continue from Settings because each command relaunched the app. No real-source success flow has been recorded.
