...
| 2026-08-25 | App verification | Simulator XCUITest now records a tab-navigation journey and iOS Debug compile is unblocked. Physical-device signing, runner-argument delivery, and broader iOS/macOS player or real-source journeys remain unverified; accessibility identifiers still cover only the golden-journey controls | Agents can prove compile plus one Simulator navigation path, but cannot treat that as device, Figma, or product-journey evidence | Record a signed physical-device XCUITest when a trusted device is available; expand identifiers only for the next bounded journey |
