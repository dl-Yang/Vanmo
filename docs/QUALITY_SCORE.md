...
| Build and agent workflow | B | `project.yml` is the configuration source; `./init.sh` provides four passing shared, boundary, documentation, and iOS UI CLI static stages; the documentation stage now rejects init stage-count and plan-index Status conflicts; focused iOS Simulator Debug compile now passes; simulator XCUITest is a separate evidence command | Default baseline still does not compile apps or run XCUITest; physical-device UI evidence is still missing |
