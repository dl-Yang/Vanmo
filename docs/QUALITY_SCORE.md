...
| Downloads | B | Download model, persistence, directory resolution, and queue logic are covered by the passing baseline; one 2026-08-26 macOS HTTP-via-Emby run passed queue, pause, restart-from-part, and main-window detail navigation; 2026-08-27 iOS simulator HTTP-via-Emby runs passed enqueue, live progress, completion, local play, and pause-terminate-`.part` resume; one 2026-08-28 physical-device SMB run passed Files-browser enqueue, same-session resume, terminate while downloading, cold-launch `.part` restore, and completion; one 2026-08-28 Vanmo-macOS SMB library-detail run passed the eleven-step mixed movie-and-episode matrix | Download journeys have no XCUITest coverage |
| Remote connections and search | C | Capability declarations and catalog playback URL behavior are covered by shared tests; 2026-08-27 Mac File Sharing runs passed connect and listing on iOS Simulator and VanmoMac; 2026-08-28 encryption-required runs on VanmoMac and an iOS device recorded SMB 3.1.1 AES-128-GCM login (`dialect=0x311`, `encrypt=true`) and a `MacShare` listing; 2026-08-28 HTTP-via-Emby catalog fetch and play ran on VanmoMac and an iOS device | Several UI-visible protocols are placeholders or incomplete; no automated real-service suite |
| Playback and prefetching | B | Catalog URL resolution is covered and cleanup paths are documented; 2026-08-27 SMB play passed on iOS Simulator via KSPlayer and on VanmoMac via the SMB-backed prefetch proxy; 2026-08-28 encrypted SMB play recorded VanmoMac prefetch `source=smb` and an iOS-device KSPlayer `smb://` load; 2026-08-28 HTTP-via-Emby play recorded VanmoMac prefetch `source=http` with seeks/pause/next-episode and an iOS-device `playing`/`buffering` path | No automated app playback, gesture, or player-window journey; iOS and macOS SMB play paths remain intentionally different; a macOS Emby seek logged `seek(to:)` continuation misuse |
