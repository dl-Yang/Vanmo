...
try {
  ...
  graph.layers.forEach(layer => {
    if (!layer.id || !layer.name || !layer.description || !Array.isArray(layer.nodeIds)) issues.push(`Layer '${layer.id || '<missing>'}' missing required fields`);
  ...
  graph.tour.forEach((step, i) => {
    if (!step.order || !step.title || !step.description || !Array.isArray(step.nodeIds)) issues.push(`Tour step[${i}] missing required fields`);
